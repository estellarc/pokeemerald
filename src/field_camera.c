...
static const u16 sTilesTransparent[] =
{
    TILE_TRANSPARENT,
    TILE_TRANSPARENT,
    TILE_TRANSPARENT,
...
void DrawDoorMetatileAt(int x, int y, u16 *tiles)
{
    ...
    if (offset >= 0)
    {
        DrawMetatile(UCHAR_MAX, tiles, offset);
        sFieldCameraOffset.copyBGToVRAM = TRUE;
    }
}
...
{
    for (u32 offsetIndex = 0; offsetIndex < 4; offsetIndex++)
        buffer[offset + sOffsetArray[offsetIndex]] = tiles[tileIndex++];
}
...
static void DrawMetatile(s32 metatileLayerType, const u16 *tiles, u16 offset)
{
    if (metatileLayerType == UCHAR_MAX)
    {
        ...
        DrawMetatileLayer(gOverworldTilemapBuffer_Bg2, sTilesTransparent, offset, 0);
